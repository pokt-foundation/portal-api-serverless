...
import { ApiGatewayResponse, newErrorResponse, newJSONResponse } from '../common/apigateway/apigateway-response'
...
export class PocketRelayer {
  ...
  checkDebug: boolean
  altruists: Record<string, string>
  aatPlan: string
  ...
  async sendRelay({
    ...
    for (let x = 0; x <= this.relayRetries; x++) {
      ...
      if (overallTimeOut && overallCurrentElasped > overallTimeOut) {
        console.log('error', 'Overall Timeout exceeded: ' + overallTimeOut, {
          requestID: requestID,
          relayType: 'APP',
          typeID: application.id,
          serviceNode: '',
        })
        return newErrorResponse(HttpStatusCode.GATEWAY_TIMEOUT, 'Overall Timeout exceeded: ' + overallTimeOut)
      }
      ...
      if (!(relayResponse instanceof Error)) {
        ...
        if (
          ...
          return newJSONResponse(HttpStatusCode.OK, JSON.parse(relayResponse.payload))
        }
        return {
    ...
    //     if (this.checkDebug) {
    //       console.log('debug', JSON.stringify(fallbackResponse.data), {
    //         requestID: requestID,
    //         relayType: 'FALLBACK',
    //         typeID: application.id,
    ...
    //     } else {
    //       console.log('error', JSON.stringify(fallbackResponse), {
    //         requestID: requestID,
    //         relayType: 'FALLBACK',
    //         typeID: application.id,
    //         serviceNode: 'fallback:' + redactedAltruistURL,
    ...
    //   }
    // }
    return newErrorResponse(HttpStatusCode.GATEWAY_TIMEOUT, 'Relay attempts exhausted')
  }
  ...
  // blockchainID,
  {
    ...
    console.log('info', 'RELAYING ' + blockchain + ' req: ' + data, {
      requestID: requestID,
      relayType: 'APP',
      typeID: application.id,
      serviceNode: '',
    ...
    if (!this.checkSecretKey(application)) {
      return new Error('SecretKey does not match')
    }
    ...
    if (!this.checkWhitelist(application.gatewaySettings.whitelistOrigins, this.origin, 'explicit')) {
      return new Error('Whitelist Origin check failed: ' + this.origin)
    }
    ...
    if (pocketSession instanceof Session) {
      const nodes: Node[] = pocketSession.sessionNodes
    ...
    if (this.checkDebug) {
      console.log('debug', JSON.stringify(pocketSession), {
        requestID: requestID,
        relayType: 'APP',
        typeID: application.id,
    ...
    if (this.checkDebug) {
      console.log('debug', JSON.stringify(relayConfiguration), {
        requestID: requestID,
        relayType: 'APP',
        typeID: application.id,
        serviceNode: node?.publicKey,
      })
      console.log('debug', JSON.stringify(relayResponse), {
        requestID: requestID,
        relayType: 'APP',
        typeID: application.id,
        serviceNode: node?.publicKey,
  ...
  async loadBlockchain(): Promise<BlockchainDetails> {
    ...
    if (!cachedBlockchains) {
      blockchains = await this.blockchainRepository.getAllBlockchains()
      await this.redis.set('blockchains', JSON.stringify(blockchains), 'EX', 600)
    } else {
    ...
    const blockchainRequest = this.host
    ...
    if (blockchainFilter[0]) {
      ...
      let blockchainID = ''
      const blockchain = blockchainFilter[0].id as string
      ...
      throw new Error('Incorrect blockchain: ' + this.host)
    }
  }

  checkSecretKey(application: Application): boolean {
