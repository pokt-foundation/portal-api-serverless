...
export class CherryPicker {
  ...
  async cherryPickApplication(
    ...
    if (rankedItems.length === 0) {
      console.log('warn', 'Cherry picking failure -- apps', {
        requestID: requestID,
        relayType: 'LB',
    ...
    if (this.checkDebug) {
      console.log('debug', 'Number of weighted applications for selection: ' + rankedItems.length, {
        requestID: requestID,
        relayType: 'LB',
        typeID: loadBalancerID,
      })
      console.log('debug', 'Selected ' + selectedApplication + ' : ' + application, {
        requestID: requestID,
        relayType: 'LB',
        typeID: loadBalancerID,
      })
  ...
  async cherryPickNode(application: Applications, nodes: Node[], blockchain: string, requestID: string): Promise<Node> {
    ...
    if (rankedItems.length === 0) {
      console.log('warn', 'Cherry picking failure -- nodes', {
        requestID: requestID,
        relayType: 'APP',
    ...
    if (this.checkDebug) {
      console.log('debug', 'Number of weighted nodes for selection: ' + rankedItems.length, {
        requestID: requestID,
        relayType: 'APP',
        typeID: application.id,
        serviceNode: '',
      })
      console.log('debug', 'Selected ' + selectedNode + ' : ' + node.publicKey, {
        requestID: requestID,
        relayType: 'APP',
        typeID: application.id,
        serviceNode: '',
  ...
  async createUnsortedLog(id: string, blockchain: string, rawServiceLog: string): Promise<ServiceLog> {
    ...
    if (clientTypeLog && clientTypeLog.includes('OpenEthereum')) {
        console.log('info', 'OPENETHEREUM MARKED', {requestID: '', relayType: '', typeID: '', serviceNode: id});
  ...
  sortLogs(array: ServiceLog[], requestID: string, relayType: string, typeID: string): ServiceLog[] {
    ...
    if (this.checkDebug) {
      console.log('debug', 'Sorted logs: ' + JSON.stringify(sortedLogs), {
        requestID: requestID,
        relayType: relayType,
        typeID: typeID,
        serviceNode: '',
