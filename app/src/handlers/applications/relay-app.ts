...
import { Pocket, HttpRpcProvider, Configuration, Account } from '@pokt-network/pocket-js'
...
const pocketSessionBlockFrequency = process.env['POCKET_SESSION_BLOCK_FREQUENCY'] || ''
...
exports.handler = async (event: ApiGatewayEvent): Promise<ApiGatewayResponse> => {
  try {
    ...
    const appRepository = new ApplicationDynamoClientRepository(process.env['APPS_TABLE_NAME'])
    const blockchainRepository = new BlockchainDynamoClientRepository(process.env['BLOCKCHAINS_TABLE_NAME'])
